...
func main() {
	...
	fmt.Println("Enter an integer betwin 1 and 100 : ")
	for {
		...
		if err != nil || (num < 1 || num > 100) {
			...
			if secret-5 <= min && num < secret {
				...
				fmt.Println("You are somewat close from the left.")
			} else if num < secret-20 {
				fmt.Println("You are way off from the left.")
			} else if secret+5 >= min && num > secret {
				fmt.Println("You are extremely close from the right.")
			} else if secret+10 >= num && num > secret+5 {
				fmt.Println("You are very close from the right.")
			} else if secret+20 >= num && num > secret+10 {
				fmt.Println("You are somewat close from the right.")
			} else if secret == num {
				messege := fmt.Sprintf("Congrats!! , You got the number after %d attempts!!! \n", len(attempts))
				fmt.Println(messege)
				break
			}
		}
